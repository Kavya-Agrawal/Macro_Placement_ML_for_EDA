...
def build_graph(node_info, node_to_net_dict):
    ...
    x = []
    for n in node_names:
        area = node_info[n]['x'] * node_info[n]['y']
        degree = len(node_to_net_dict[n])
        x.append([area, degree, node_info[n]['x'], node_info[n]['y']])
    ...
    for n in node_names:
        i = name_to_idx[n]
        for net in node_to_net_dict[n]:
            for m in node_to_net_dict:
                if net in node_to_net_dict[m]:
                    j = name_to_idx[m]
                    adj[i][j] = 1.0
