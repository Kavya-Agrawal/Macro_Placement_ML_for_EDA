...
def get_node_id_to_name_topology(node_info, node_to_net_dict, net_info, benchmark):
    ...
    while len(node_id_to_name) < len(node_info):
        ...
        if len(candidates) > 0:
            ## chnaged here removed hash and added sum- 189,192,195
            if benchmark != 'ariane':
                if benchmark == "bigblue3":
                    add_node = max(candidates, key = lambda v: candidates[v]*1 + node_net_num[v]*100000 +\
                        node_info[v]['x']*node_info[v]['y'] * 1 +sum(ord(c) for c in v) * 1e-6)
                else:
...
#     try:
#         model.load_state_dict(torch.load("gnn_ordering_model.pth", map_location=device))
#     except:
#         print("WARNING: model weights not found, using random model")

#     model.eval()

#     # ---- Use your existing policy (no changes) ----
...
#     # ---- Update node IDs (same behavior as original function) ----
#     for i, node_name in enumerate(ordering):
#         node_info[node_name]["id"] = i
