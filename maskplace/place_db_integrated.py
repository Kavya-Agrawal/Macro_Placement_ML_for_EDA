...
# Import RL ordering if available
try:
    from rl_ordering import RLOrderingAgent
    RL_AVAILABLE = True
except ImportError:
    RL_AVAILABLE = False
    print("Warning: RL ordering not available. Using heuristic ordering only.")
...
def get_node_id_to_name(node_info, node_to_net_dict):
    """Simple ordering by connectivity"""
    node_name_and_num = []
    for node_name in node_info:
...
def get_node_id_to_name_topology(node_info, node_to_net_dict, net_info, benchmark):
    """Original heuristic-based topology ordering"""
    node_id_to_name = []
    adjacency = {}
    for net_name in net_info:
...
def get_node_id_to_name_rl(placedb, rl_agent=None, checkpoint_path=None):
    """RL-based ordering using trained actor-critic model"""
    
    if not RL_AVAILABLE:
        print("RL ordering not available. Falling back to heuristic ordering.")
        return get_node_id_to_name_topology(
            placedb.node_info, 
    ...
    if rl_agent is None:
        rl_agent = RLOrderingAgent(placedb)
    ...
    # Generate ordering
    ordering = rl_agent.generate_ordering(training=False)
    
    # Convert to node names and update IDs
    node_id_to_name = [placedb.node_id_to_name[idx] for idx in ordering]
    
    for i, node_name in enumerate(node_id_to_name):
        placedb.node_info[node_name]["id"] = i
...
class PlaceDB:
    ...
    def __init__(self, benchmark="adaptec1", ordering_method="topology", rl_checkpoint=None):
        """
        Args:
            benchmark: Benchmark name/path
            ordering_method: 'topology' (heuristic), 'rl' (reinforcement learning), or 'simple' (by connectivity)
            rl_checkpoint: Path to RL model checkpoint (only used if ordering_method='rl')
        """
        ...
        # Create temporary node_id_to_name for RL agent initialization
        temp_ordering = list(self.node_info.keys())
        for i, node_name in enumerate(temp_ordering):
            self.node_info[node_name]["id"] = i
        self.node_id_to_name = temp_ordering
        
        # Select ordering method
        if ordering_method == "rl":
            print("Using RL-based ordering")
            self.node_id_to_name = get_node_id_to_name_rl(self, checkpoint_path=rl_checkpoint)
        elif ordering_method == "topology":
            print("Using topology-based heuristic ordering")
            self.node_id_to_name = get_node_id_to_name_topology(
                self.node_info, self.node_to_net_dict, self.net_info, self.benchmark
            )
        elif ordering_method == "simple":
            print("Using simple connectivity-based ordering")
    ...
    def debug_str(self):
        ...
        print("ordering_method = {}".format(self.ordering_method))
        print("First 10 nodes in ordering:")
        for i, node_name in enumerate(self.node_id_to_name[:10]):
            print(f"  {i}: {node_name}")


if __name__ == "__main__":
    # Test with different ordering methods
    
    print("=" * 60)
    print("Testing HEURISTIC ordering")
    print("=" * 60)
    placedb_heuristic = PlaceDB("ariane", ordering_method="topology")
    placedb_heuristic.debug_str()
    
    print("\n" + "=" * 60)
    print("Testing RL ordering (requires training first)")
    print("=" * 60)
    # First train a model or use existing checkpoint
    # placedb_rl = PlaceDB("ariane", ordering_method="rl", rl_checkpoint="rl_checkpoints/best_model.pt")
    # placedb_rl.debug_str()
